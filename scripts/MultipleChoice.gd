extends Control
## Space Invaders Multiple Choice Game
...
func _setup_game_area() -> void:
	...
	# Create question label at top (centered)
	...
	question_label.add_theme_color_override("font_color", Colors.LIGHT_BASE)
	game_area.add_child(question_label)
...
func _display_question() -> void:
	...
	question_label.text = "What does \"" + q.word + "\" mean?"
...
func _create_word_ships(q: Question) -> void:
	...
	for i in range(q.options.size()):
		...
		ship_container.size = Vector2(150, 120)
		ship_container.position = Vector2(start_offset + ship_spacing * (i + 1) - 75, ship_y)
		...
		if ship_texture:
			...
			ship_sprite.position = Vector2(75, 40)  # Center in container
		...
		ship_container.size = Vector2(150, 135)
