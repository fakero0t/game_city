extends Control
## Space Invaders Fill-in-the-Blank Game
...
func _setup_game_area() -> void:
	...
	# Create sentence label at top (centered)
	sentence_label = Label.new()
	sentence_label.name = "SentenceLabel"
	var viewport_size = get_viewport_rect().size
	sentence_label.position = Vector2(50, 40)
	sentence_label.size = Vector2(viewport_size.x - 100, 80)
	sentence_label.add_theme_font_size_override("font_size", 28)
	sentence_label.autowrap_mode = TextServer.AUTOWRAP_WORD
...
func _process(delta: float) -> void:
	if not current_question.has("options") or not is_game_active:
		return
...
# Removed _animate_engine_pulse - no longer needed with sprite-based ships

# Removed _animate_base_engines - no longer needed with sprite-based player
...
func load_activity_data(activity_data: Dictionary) -> void:
	...
	current_question = {
		...
		"correct_word": word_data.get("headword", ""),
		"options": []
	}
	...
	for option in options_array:
		current_question["options"].append(str(option))
	
	# Shuffle options
	current_question["options"].shuffle()
	
	# Find correct index
	current_question["correct_index"] = current_question["options"].find(current_question["correct_word"])
	if current_question["correct_index"] == -1:
		current_question["correct_index"] = 0
...
func _create_word_ships() -> void:
	...
	for i in range(current_question["options"].size()):
		var word = current_question["options"][i]
		var is_correct = (word == current_question["correct_word"])
		...
		if ship_texture:
			...
			if texture_size.x > 0 and texture_size.y > 0:
				var scale_factor = min(100.0 / texture_size.y, 100.0 / texture_size.x)
				ship_sprite.scale = Vector2(scale_factor, scale_factor)
			ship_sprite.position = Vector2(60, 50)  # Center in container
		...
		word_label.position = Vector2(0, 105)
		...
		ship_container.size = Vector2(120, 105)
