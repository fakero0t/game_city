extends Control
## Space Invaders Synonym/Antonym Game
...
func _ready() -> void:
	...
	# Note: No Character node in this scene, so skip adding to it
	...
	video_player.position = Vector2(180, 400) - Vector2(100, 100)
...
func _display_question() -> void:
	...
	question_label.text = "Which word is a SYNONYM for \"" + q.target_word + "\"?"
...
func _create_word_ships(q: RelationshipQuestion) -> void:
	...
	for i in range(q.options.size()):
		...
		var is_correct = (word == q.correct_answer)
